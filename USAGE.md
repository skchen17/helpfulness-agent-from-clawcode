...
- One of:
  ...
  - `OPENAI_API_KEY` for OpenAI-compatible endpoints (including LM Studio)
  - `claw login` for OAuth-based auth
- Optional base URL override:
  - `ANTHROPIC_BASE_URL` when targeting an Anthropic proxy or local shim
  - `OPENAI_BASE_URL` when targeting OpenAI-compatible services (for example `http://127.0.0.1:1234/v1`)
...
When LM Studio is running a local model with OpenAI-compatible API enabled, configure credentials and base URL in a config file:
...
Recommended location for machine-local values: `<repo>/.claw/settings.local.json`.

Then run:

```bash
cd rust
./target/debug/claw --model qwen2.5-coder-7b-instruct
```
