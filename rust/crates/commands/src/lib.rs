...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "orchestrate",
        aliases: &["orch"],
        summary: "Run the Python multi-agent orchestrator for a task",
    ...
    SlashCommandSpec {
        name: "image",
        aliases: &[],
        summary: "Add one or more image files to the conversation",
        argument_hint: Some("<path ...>"),
        resume_supported: false,
    },
    SlashCommandSpec {
...
pub fn validate_slash_command_input(
    ...
    Ok(Some(match command {
        ...
        "orchestrate" | "orch" => SlashCommand::Orchestrate {
            objective: Some(require_remainder(command, remainder, "<objective>")?),
        },
        "doctor" => {
            validate_no_args(command, &args)?;
            SlashCommand::Doctor
        ...
        "image" => SlashCommand::Image { path: remainder },
        "exit" => {
            validate_no_args(command, &args)?;
            SlashCommand::Exit
        }
...
fn slash_command_category(name: &str) -> &'static str {
    match name {
        ...
        | "orchestrate" => "Analysis & automation",
...
mod tests {
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/image"),
            Ok(Some(SlashCommand::Image { path: None }))
        );
        assert_eq!(
            SlashCommand::parse("/image docs/diagram.png"),
            Ok(Some(SlashCommand::Image {
                path: Some("docs/diagram.png".to_string())
            }))
        );
        assert_eq!(
            SlashCommand::parse("/image docs/a.png docs/b.png"),
            Ok(Some(SlashCommand::Image {
                path: Some("docs/a.png docs/b.png".to_string())
            }))
        );
        assert_eq!(
    ...
    fn renders_help_from_shared_specs() {
        ...
        assert!(help.contains("/orchestrate <objective>"));
    ...
    #[test]
    fn parses_orchestrate_slash_command_and_requires_objective() {
        assert_eq!(
            validate_slash_command_input("/orchestrate summarize recent memory changes"),
            Ok(Some(SlashCommand::Orchestrate {
                objective: Some("summarize recent memory changes".to_string()),
            }))
        );

        assert_eq!(
            validate_slash_command_input("/orch inspect current task routing"),
            Ok(Some(SlashCommand::Orchestrate {
                objective: Some("inspect current task routing".to_string()),
            }))
        );

        let error = validate_slash_command_input("/orchestrate")
            .expect_err("missing orchestrate objective should be rejected")
            .to_string();
        assert!(error.contains("Usage: /orchestrate <objective>"));
    }
