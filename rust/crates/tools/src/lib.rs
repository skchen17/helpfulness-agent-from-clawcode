...
use runtime::{
    ...
    grep_search, load_system_prompt, ConfigLoader,
...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "TaskCreate",
            description: "Create a background task and auto-start it as a sub-agent lane by default (set auto_start=false to only queue).",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "prompt": { "type": "string" },
                    ...
                    "auto_start_subagent_type": { "type": "string" },
                    "auto_start_model": { "type": "string" }
                },
                "required": ["prompt"],
                "additionalProperties": false
            }),
        ...
        ToolSpec {
            ...
            input_schema: json!({
                ...
                "properties": {
                    "task_id": { "type": "string" },
                    "message": { "type": "string" },
                    "auto_activate_created": { "type": "boolean" }
                },
        ...
        ToolSpec {
            name: "TeamCreate",
            description: "Create a team and attach tasks by task_id or prompt. Optionally auto-start each task as a background sub-agent.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "name": { "type": "string" },
        ...
        ToolSpec {
            name: "TeamDelete",
            description: "Delete a team and stop non-terminal tasks that are assigned to it.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "team_id": { "type": "string" }
...
fn run_task_create(input: TaskCreateInput) -> Result<String, String> {
    run_task_create_with_agent_launcher(input, execute_agent)
}

fn run_task_create_with_agent_launcher<F>(
    input: TaskCreateInput,
    mut agent_launcher: F,
) -> Result<String, String>
where
    F: FnMut(AgentInput) -> Result<AgentOutput, String>,
{
    let TaskCreateInput {
        prompt,
        description,
        auto_start,
        auto_start_subagent_type,
        auto_start_model,
    } = input;

    let registry = global_task_registry();
    let task = registry.create(&prompt, description.as_deref());

    let mut auto_started_agent: Option<Value> = None;
    let mut auto_start_failure: Option<Value> = None;
    ...
    if auto_start {
        ...
        let agent_input = AgentInput {
            ...
            prompt: prompt.clone(),
            subagent_type: auto_start_subagent_type,
            name: Some(format!("task-{}", task.task_id)),
            model: auto_start_model,
            task_id: Some(task.task_id.clone()),
        };

        match agent_launcher(agent_input) {
            Ok(agent) => {
                let agent_id = agent.agent_id.clone();
                let launcher_status = agent.status.clone();
                let output_file = agent.output_file.clone();
                let manifest_file = agent.manifest_file.clone();

                match probe_agent_launch_state(&manifest_file) {
                    Ok(probe) => {
                        let observed_status = probe.status;
                        match task_status_from_agent_status(&observed_status) {
                            Some(TaskStatus::Failed) => {
                                let reason = probe.error.unwrap_or_else(|| {
                                    "agent entered failed state during launch probe".to_string()
                                });
                                let _ = registry.set_status(&task.task_id, TaskStatus::Failed);
                                auto_start_failure = Some(json!({
                            ...
                            Some(TaskStatus::Completed) => {
                                let _ = registry.set_status(&task.task_id, TaskStatus::Completed);
                                auto_started_agent = Some(json!({
                                    "task_id": task.task_id,
                                    "agent_id": agent_id,
                                    "launcher_status": launcher_status,
                                    "observed_status": observed_status,
                                    "derived_state": probe.derived_state,
                                    "output_file": output_file,
                                    "manifest_file": manifest_file
                                }));
                            }
                            _ => {
                                let _ = registry.set_status(&task.task_id, TaskStatus::Running);
                                auto_started_agent = Some(json!({
                                    "task_id": task.task_id,
                                    "agent_id": agent_id,
                                    "launcher_status": launcher_status,
    ...
    let snapshot = registry.get(&task.task_id).unwrap_or(task);
    let model_max_parallel_per_model = configured_agent_model_max_parallel_per_model();
    let model_execution_policy = if model_max_parallel_per_model == 1 {
        "single_lane_per_model"
    } else {
        "bounded_parallel_per_model"
    };
    let execution_mode = if auto_start { "auto_start" } else { "manual" };
    let next_step_hint = if auto_start {
        if auto_start_failure.is_none() {
            "Task created and auto-started. Inspect agent manifests under .clawd-agents."
        } else {
            "Task created but auto-start failed. Inspect auto_start_failure and retry."
        }
    } else {
        "Task queued only. Launch via Agent/Worker tools when ready."
    };
    ...
    to_pretty_json(json!({
        ...
        "auto_start_started": auto_started_agent.is_some(),
        "auto_started_agent": auto_started_agent,
        "auto_start_failure": auto_start_failure,
        "model_execution_policy": model_execution_policy,
        "model_max_parallel_per_model": model_max_parallel_per_model,
...
fn run_task_update(input: TaskUpdateInput) -> Result<String, String> {
    let TaskUpdateInput {
        task_id,
        message,
        auto_activate_created,
    } = input;

    let registry = global_task_registry();
    match registry.update(&task_id, &message) {
        Ok(task) => {
            let mut activation_attempted = false;
            let mut activation_started = false;
            let mut activated_agent: Option<Value> = None;
            let mut activation_failure: Option<Value> = None;
            let mut activation_skipped_reason: Option<String> = None;
            let activation_eligible =
                matches!(task.status, TaskStatus::Created | TaskStatus::Failed | TaskStatus::Stopped);

            if auto_activate_created && activation_eligible {
                ...
                let merged_prompt = if message.trim().is_empty() {
                    task.prompt.clone()
                } else {
                    format!(
                        "{}\n\nAdditional instruction from TaskUpdate:\n{}",
                        task.prompt, message
                    )
                };
                ...
                match execute_agent(agent_input) {
                    Ok(agent) => {
                        let agent_id = agent.agent_id.clone();
                        let launcher_status = agent.status.clone();
                        let output_file = agent.output_file.clone();
                        ...
                        match probe_agent_launch_state(&manifest_file) {
                            Ok(probe) => {
                                ...
                                match task_status_from_agent_status(&observed_status) {
                                    Some(TaskStatus::Failed) => {
                                        let reason = probe.error.unwrap_or_else(|| {
                                            "agent entered failed state during launch probe"
                                                .to_string()
                                        });
                                        let _ =
                                            registry.set_status(&task.task_id, TaskStatus::Failed);
                                        activation_failure = Some(json!({
                                            "task_id": task.task_id,
                                            "agent_id": agent_id,
                                            "manifest_file": manifest_file,
                                            "reason": reason
            ...
            } else if !auto_activate_created {
                activation_skipped_reason = Some(String::from(
                    "auto_activate_created=false; skipping activation",
                ));
            } else {
                activation_skipped_reason = Some(String::from(
                    "task is not in an activatable state (created/failed/stopped)",
                ));
            }
            ...
            to_pretty_json(json!({
                ...
                "activated_agent": activated_agent,
...
fn run_team_create(input: TeamCreateInput) -> Result<String, String> {
    run_team_create_with_agent_launcher(input, execute_agent)
}

fn run_team_create_with_agent_launcher<F>(
    ...
    F: FnMut(AgentInput) -> Result<AgentOutput, String>,
{
    ...
    if tasks.is_empty() {
        return Err("team must include at least one task".to_string());
    }
    ...
    for task in tasks {
        ...
        let prompt = task
            ...
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .ok_or_else(|| "each team task must include either task_id or prompt".to_string())?;

        let created_task = task_registry.create(prompt, task.description.as_deref());
        task_ids.push(created_task.task_id);
    }
    ...
    let mut auto_started_agents = Vec::new();
    let mut auto_start_failures = Vec::new();
    if auto_start {
        for (index, task_id) in team.task_ids.iter().enumerate() {
            let Some(task) = task_registry.get(task_id) else {
                auto_start_failures.push(json!({
                    "task_id": task_id,
            ...
            if task.status != TaskStatus::Created {
                auto_start_failures.push(json!({
                    "task_id": task_id,
                    "reason": format!(
                        "task is not eligible for auto-start from status {}",
                        task.status
                    )
                }));
                continue;
            ...
            if prompt.is_empty() {
                auto_start_failures.push(json!({
                    "task_id": task_id,
                    "reason": "task prompt is empty"
                }));
                continue;
            }

            let agent_name = format!("{}-{}", name, index + 1);
            let agent_description = format!("Team {} task {}", team.name, task_id);
            let agent_input = AgentInput {
                description: agent_description,
                prompt: prompt.to_string(),
                subagent_type: auto_start_subagent_type.clone(),
                name: Some(agent_name),
                model: auto_start_model.clone(),
            ...
            match agent_launcher(agent_input) {
                Ok(agent) => {
                    ...
                    match probe_agent_launch_state(&manifest_file) {
                        Ok(probe) => {
                            let observed_status = probe.status.clone();
                            match task_status_from_agent_status(&observed_status) {
                                Some(TaskStatus::Failed) => {
                                    let reason = probe.error.unwrap_or_else(|| {
                                        "agent entered failed state during launch probe"
                                    ...
                                    let _ = task_registry.set_status(task_id, TaskStatus::Failed);
                                    auto_start_failures.push(json!({
                                        "task_id": task_id,
                                        "agent_id": agent_id,
                                        "manifest_file": manifest_file,
                                ...
                                Some(TaskStatus::Completed) => {
                                    let _ = task_registry.set_status(task_id, TaskStatus::Completed);
                                }
                                _ => {
                                    let _ = task_registry.set_status(task_id, TaskStatus::Running);
                                }
                            }

                            auto_started_agents.push(json!({
                                "task_id": task_id,
                                "agent_id": agent_id,
                                "launcher_status": launcher_status,
                                "observed_status": observed_status,
    ...
    let execution_mode = if auto_start { "auto_start" } else { "manual" };
    let next_step_hint = if auto_start {
        if auto_start_failures.is_empty() {
            "Team created and auto-started. Inspect agent manifests under .clawd-agents."
        } else {
            "Team created with partial auto-start failures. Inspect auto_start_failures and retry failed lanes."
        }
    } else {
        "Use Agent/Worker tools or /orchestrate to run tasks."
    };
    ...
    to_pretty_json(json!({
        ...
        "model_execution_policy": model_execution_policy,
        "model_max_parallel_per_model": model_max_parallel_per_model,
        "auto_start_started_count": auto_started_agents.len(),
        "auto_start_failed_count": auto_start_failures.len(),
        "auto_started_agents": auto_started_agents,
        "auto_start_failures": auto_start_failures,
        "next_step_hint": next_step_hint
    }))
}
...
fn run_team_delete(input: TeamDeleteInput) -> Result<String, String> {
    match global_team_registry().delete(&input.team_id) {
        Ok(team) => {
            let task_registry = global_task_registry();
            let mut stopped_task_count = 0_usize;
            let mut terminal_task_count = 0_usize;
            let mut missing_task_count = 0_usize;
            ...
            for task_id in &team.task_ids {
                ...
                match task.status {
                    TaskStatus::Created | TaskStatus::Running => {
                        if task_registry.stop(task_id).is_ok() {
...
struct TaskCreateInput {
    ...
    #[serde(default)]
    description: Option<String>,
    #[serde(default = "default_task_create_auto_start")]
    auto_start: bool,
    #[serde(default)]
    auto_start_subagent_type: Option<String>,
    #[serde(default)]
    auto_start_model: Option<String>,
}

const fn default_task_create_auto_start() -> bool {
    true
}
...
struct TaskUpdateInput {
    task_id: String,
    message: String,
    #[serde(default = "default_task_update_auto_activate_created")]
    auto_activate_created: bool,
}

const fn default_task_update_auto_activate_created() -> bool {
    true
}
...
struct AgentJob {
    ...
    upstream_output_targets: Vec<String>,
}
...
const DEFAULT_AGENT_MAX_PARALLEL_PER_MODEL: usize = 1;
const TEAM_AUTO_START_MANIFEST_PROBE_TIMEOUT: Duration = Duration::from_millis(900);
const TEAM_AUTO_START_MANIFEST_PROBE_INTERVAL: Duration = Duration::from_millis(90);

#[derive(Debug)]
struct AgentLaunchProbe {
    status: String,
    error: Option<String>,
...
struct AgentSpawnContext {
    task_id: Option<String>,
    upstream_output_targets: Vec<String>,
}

thread_local! {
    static AGENT_SPAWN_CONTEXT: RefCell<Option<AgentSpawnContext>> = const { RefCell::new(None) };
}

struct AgentSpawnContextGuard {
    previous: Option<AgentSpawnContext>,
}

impl AgentSpawnContextGuard {
    fn enter(context: AgentSpawnContext) -> Self {
        let previous = AGENT_SPAWN_CONTEXT.with(|slot| slot.replace(Some(context)));
        Self { previous }
    }
}

impl Drop for AgentSpawnContextGuard {
    fn drop(&mut self) {
        let previous = self.previous.take();
        AGENT_SPAWN_CONTEXT.with(|slot| {
            slot.replace(previous);
        });
    }
}

fn current_agent_spawn_context() -> Option<AgentSpawnContext> {
    AGENT_SPAWN_CONTEXT.with(|slot| slot.borrow().clone())
}

#[derive(Debug)]
struct AgentModelConcurrencyGate {
    state: std::sync::Mutex<AgentModelConcurrencyState>,
    condvar: std::sync::Condvar,
}

#[derive(Debug)]
struct AgentModelConcurrencyState {
    active_per_model: BTreeMap<String, usize>,
    max_parallel_per_model: usize,
}

#[derive(Debug)]
struct AgentModelConcurrencyPermit<'a> {
    gate: &'a AgentModelConcurrencyGate,
    model_key: String,
}

impl AgentModelConcurrencyGate {
    fn new(max_parallel_per_model: usize) -> Self {
        Self {
            state: std::sync::Mutex::new(AgentModelConcurrencyState {
                active_per_model: BTreeMap::new(),
    ...
    fn acquire(&self, model: &str) -> AgentModelConcurrencyPermit<'_> {
        ...
        loop {
            let active = state.active_per_model.get(&model_key).copied().unwrap_or(0);
            if active < state.max_parallel_per_model {
                break;
            }
            state = self
                .condvar
        ...
        *state.active_per_model.entry(model_key.clone()).or_insert(0) += 1;
        AgentModelConcurrencyPermit {
            gate: self,
            model_key,
        }
    }

    fn release(&self, model_key: &str) {
        let mut state = self
            .state
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let should_remove = match state.active_per_model.get_mut(model_key) {
        ...
        if should_remove {
            state.active_per_model.remove(model_key);
        }
        self.condvar.notify_all();
    }
}

impl Drop for AgentModelConcurrencyPermit<'_> {
    fn drop(&mut self) {
        self.gate.release(&self.model_key);
    }
}

fn configured_agent_model_max_parallel_per_model() -> usize {
    std::env::var("CLAWD_AGENT_MAX_PARALLEL_PER_MODEL")
        .ok()
        .and_then(|raw| raw.trim().parse::<usize>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(DEFAULT_AGENT_MAX_PARALLEL_PER_MODEL)
}

fn global_agent_model_concurrency_gate() -> &'static AgentModelConcurrencyGate {
    use std::sync::OnceLock;
    static GATE: OnceLock<AgentModelConcurrencyGate> = OnceLock::new();
    GATE.get_or_init(|| {
        AgentModelConcurrencyGate::new(configured_agent_model_max_parallel_per_model())
    })
}

fn probe_agent_launch_state(manifest_file: &str) -> Result<AgentLaunchProbe, String> {
    let deadline = Instant::now() + TEAM_AUTO_START_MANIFEST_PROBE_TIMEOUT;
    loop {
        match read_agent_launch_probe(manifest_file) {
            Ok(probe) => {
                if matches!(
                    task_status_from_agent_status(&probe.status),
                    Some(TaskStatus::Failed | TaskStatus::Completed)
                ) {
                    return Ok(probe);
                }
        ...
        std::thread::sleep(TEAM_AUTO_START_MANIFEST_PROBE_INTERVAL);
    }
}

fn read_agent_launch_probe(manifest_file: &str) -> Result<AgentLaunchProbe, String> {
    let contents = std::fs::read_to_string(manifest_file)
        .map_err(|error| format!("failed to read agent manifest `{manifest_file}`: {error}"))?;
    let parsed: Value = serde_json::from_str(&contents)
        .map_err(|error| format!("failed to parse agent manifest `{manifest_file}`: {error}"))?;
    let status = parsed
        .get("status")
        .and_then(Value::as_str)
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| format!("agent manifest `{manifest_file}` missing status"))?
        .to_string();
    ...
    F: FnOnce(AgentJob) -> Result<(), String>,
{
    ...
    std::fs::create_dir_all(&output_dir).map_err(|error| {
        format!(
            "failed to create agent store directory `{}`: {error}",
            output_dir.display()
        )
    })?;
    ...
    apply_runtime_env_overrides_for_agent_runtime()?;
    ...
    let parent_context = current_agent_spawn_context();
    let task_id = input
        .task_id
        .clone()
        .or_else(|| parent_context.as_ref().and_then(|context| context.task_id.clone()));
    let upstream_output_targets = parent_context
        .map(|context| context.upstream_output_targets)
        .unwrap_or_default();
...
",
    ...
    std::fs::write(&output_file, output_contents).map_err(|error| {
        format!(
            "failed to write agent output file `{}`: {error}",
            output_file.display()
        )
    })?;
    ...
    let job = AgentJob {
        ...
        system_prompt,
        allowed_tools,
        upstream_output_targets: upstream_output_targets.clone(),
    };
    if let Err(error) = spawn_fn(job) {
        let error = format!("failed to spawn sub-agent: {error}");
        persist_agent_terminal_state_with_targets(
            &manifest,
            "failed",
            None,
            Some(error.clone()),
            &upstream_output_targets,
        )?;
        return Err(error);
    }
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            match result {
                Ok(Ok(())) => {}
                Ok(Err(error)) => {
                    let _ = persist_agent_terminal_state_with_targets(
                        &job.manifest,
                        "failed",
                        None,
                        Some(error),
                        &job.upstream_output_targets,
                    );
                }
                Err(_) => {
                    let _ = persist_agent_terminal_state_with_targets(
...
fn run_agent_job(job: &AgentJob) -> Result<(), String> {
    let _spawn_context_guard = AgentSpawnContextGuard::enter(build_child_agent_spawn_context(job));
    let model = job.manifest.model.as_deref().unwrap_or(DEFAULT_AGENT_MODEL);
    let _model_permit = global_agent_model_concurrency_gate().acquire(model);
...
fn build_child_agent_spawn_context(job: &AgentJob) -> AgentSpawnContext {
    let mut upstream_output_targets = job.upstream_output_targets.clone();
    if !upstream_output_targets
        .iter()
        .any(|path| path == &job.manifest.output_file)
    {
        upstream_output_targets.push(job.manifest.output_file.clone());
    }
    AgentSpawnContext {
        task_id: job.manifest.task_id.clone(),
        upstream_output_targets,
    }
}
...
fn resolve_agent_model(model: Option<&str>) -> String {
    if let Some(explicit) = model.map(str::trim).filter(|value| !value.is_empty()) {
        return explicit.to_string();
    }

    if let Some(from_env) = std::env::var("CLAWD_AGENT_DEFAULT_MODEL")
        .ok()
    ...
    if let Some(from_config) = load_runtime_model_for_agent() {
        return from_config;
    }
...
fn load_runtime_model_for_agent() -> Option<String> {
    load_runtime_config_for_agent()?
        .model()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(str::to_string)
...
fn apply_runtime_env_overrides_for_agent_runtime() -> Result<(), String> {
    let Some(config) = load_runtime_config_for_agent() else {
        return Ok(());
    };
    ...
    let env_object = env_value
        .as_object()
        .ok_or_else(|| String::from("merged settings.env must be a JSON object"))?;

    for (key, value) in env_object {
        if key.trim().is_empty() {
            return Err(String::from(
                "merged settings.env keys must be non-empty strings",
            ));
        }
        let value = value
            .as_str()
            .ok_or_else(|| format!("merged settings.env.{key} must be a string"))?;
...
fn load_runtime_config_for_agent() -> Option<runtime::RuntimeConfig> {
    let base_dir = resolve_runtime_config_base_dir_for_agent()?;
    ConfigLoader::default_for(base_dir).load().ok()
}

fn resolve_runtime_config_base_dir_for_agent() -> Option<PathBuf> {
    let cwd = std::env::current_dir().ok()?;
    if let Some(base_dir) = find_runtime_config_base_from_path(&cwd) {
        return Some(base_dir);
    }

    if let Some(base_dir) = std::env::var("CLAWD_AGENT_CONFIG_BASE_DIR")
        .ok()
        .map(PathBuf::from)
        .and_then(|path| find_runtime_config_base_from_path(&path))
    {
        return Some(base_dir);
    }

    if let Some(base_dir) = std::env::var("CLAWD_AGENT_STORE")
        .ok()
        .map(PathBuf::from)
        .and_then(|path| find_runtime_config_base_from_path(&path))
    {
    ...
    if let Some(base_dir) = std::env::current_exe()
        .ok()
        .as_deref()
...
fn write_agent_manifest(manifest: &AgentOutput) -> Result<(), String> {
    ...
    let manifest_file = normalized.manifest_file.clone();
    let manifest_json =
        serde_json::to_string_pretty(&normalized).map_err(|error| {
            format!("failed to serialize agent manifest `{manifest_file}`: {error}")
        })?;
    std::fs::write(
...
fn persist_agent_terminal_state(
    ...
    persist_agent_terminal_state_with_targets(manifest, status, result, error, &[])
}

fn persist_agent_terminal_state_with_targets(
    manifest: &AgentOutput,
    status: &str,
    result: Option<&str>,
    error: Option<String>,
    upstream_output_targets: &[String],
) -> Result<(), String> {
    ...
    append_agent_output(&manifest.output_file, &terminal_output)?;

    let mut seen_targets = BTreeSet::new();
    for target in upstream_output_targets {
        let trimmed = target.trim();
        if trimmed.is_empty() || trimmed == manifest.output_file.as_str() {
            continue;
        }
        if !seen_targets.insert(trimmed.to_string()) {
            continue;
        }
        let _ = append_agent_output(trimmed, &terminal_output);
    }
    ...
    if let (Some(task_id), Some(task_status)) = (
        next_manifest.task_id.as_deref(),
        task_status_from_agent_status(status),
    ) {
        let _ = global_task_registry().set_status(task_id, task_status);
    }
...
fn task_status_from_agent_status(status: &str) -> Option<TaskStatus> {
    match status.trim().to_ascii_lowercase().as_str() {
        "running" => Some(TaskStatus::Running),
        "completed" | "finished" => Some(TaskStatus::Completed),
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("CLAWD_AGENT_STORE") {
        ...
        if !trimmed.is_empty() {
            ...
            ensure_agent_store_writable(&explicit).map_err(|error| {
                format!(
                    "CLAWD_AGENT_STORE `{}` is not writable: {error}",
                    explicit.display()
                )
            })?;
            return Ok(explicit);
    ...
    let cwd = std::env::current_dir()
        .map_err(|error| format!("failed to resolve current directory for agent store: {error}"))?;

    let mut candidates = vec![cwd.join(".clawd-agents")];
    if let Ok(home) = std::env::var("HOME") {
        let home_trimmed = home.trim();
        if !home_trimmed.is_empty() {
            candidates.push(std::path::PathBuf::from(home_trimmed).join(".clawd-agents"));
        }
    }
    candidates.push(std::env::temp_dir().join("clawd-agents"));

    let mut failures = Vec::new();
    for candidate in candidates {
        match ensure_agent_store_writable(&candidate) {
    ...
    Err(format!(
        "failed to prepare writable agent store directory; set CLAWD_AGENT_STORE to a writable path. attempts: {}",
        failures.join(" | ")
    ))
}

fn ensure_agent_store_writable(path: &Path) -> Result<(), String> {
    std::fs::create_dir_all(path)
        .map_err(|error| format!("failed to create `{}`: {error}", path.display()))?;

    let probe = path.join(format!(".clawd-write-probe-{}", make_agent_id()));
    std::fs::write(&probe, b"probe").map_err(|error| {
        format!(
            "failed to write probe `{}` (store `{}`): {error}",
            probe.display(),
            path.display()
        )
    })?;
...
mod tests {
    ...
    use super::{
        agent_permission_policy, agent_store_dir, allowed_tools_for_subagent, classify_lane_failure,
        ...
        persist_agent_terminal_state, push_output_block, run_task_create_with_agent_launcher,
        run_task_packet, run_team_create_with_agent_launcher, AgentInput, AgentJob, AgentOutput,
        GlobalToolRegistry, LaneEventName, LaneFailureClass, SubagentToolExecutor,
        TaskCreateInput, TaskStatus, TeamCreateInput, TeamTaskInput,
    };
    ...
    fn run_git(cwd: &Path, args: &[&str]) {
        ...
        assert!(status.success(), "git command failed for args: {args:?}");
    }
    ...
    #[test]
    fn team_create_builds_tasks_from_prompts_and_assigns_team() {
        let created = execute_tool(
            "TeamCreate",
            &json!({
                "name": "Prompt Team",
                "tasks": [
                    {
                        "prompt": "Investigate flaky parity behavior",
                        "description": "lane-a"
                    },
                    {
                        "prompt": "Draft release notes for parity fixes"
                    }
                ]
            }),
        )
        .expect("TeamCreate should succeed with prompt-based tasks");
        let created_output: serde_json::Value = serde_json::from_str(&created).expect("json");
        let team_id = created_output["team_id"]
            .as_str()
            .expect("team id")
            .to_string();
        assert_eq!(created_output["task_count"], 2);

        let task_ids = created_output["task_ids"]
            .as_array()
            .expect("task ids array")
            .iter()
            .map(|value| value.as_str().expect("task id").to_string())
            .collect::<Vec<_>>();
        assert_eq!(task_ids.len(), 2);

        for task_id in task_ids {
            let task = execute_tool("TaskGet", &json!({ "task_id": task_id }))
                .expect("TaskGet should succeed for created team task");
            let task_output: serde_json::Value = serde_json::from_str(&task).expect("json");
            assert_eq!(
                task_output["team_id"],
                serde_json::Value::String(team_id.clone())
            );
            assert_eq!(task_output["status"], "created");
        }
    }

    #[test]
    fn team_create_accepts_existing_task_ids() {
        let created_task = execute_tool(
            "TaskCreate",
            &json!({
        ...
        let created_task_output: serde_json::Value =
            serde_json::from_str(&created_task).expect("json");
        let task_id = created_task_output["task_id"]
            .as_str()
            .expect("task id")
            .to_string();
        ...
        .expect("TeamCreate should accept task_id");
        let team_output: serde_json::Value = serde_json::from_str(&created_team).expect("json");
        let team_id = team_output["team_id"].as_str().expect("team id").to_string();

        let listed_task_id = team_output["task_ids"][0]
            .as_str()
            .expect("listed task id");
        assert_eq!(listed_task_id, task_id);

        let task = execute_tool("TaskGet", &json!({ "task_id": listed_task_id }))
            .expect("TaskGet should succeed");
        let task_output: serde_json::Value = serde_json::from_str(&task).expect("json");
        assert_eq!(task_output["team_id"], serde_json::Value::String(team_id));
    }

    #[test]
    fn team_delete_stops_non_terminal_team_tasks() {
        let created_team = execute_tool(
            "TeamCreate",
            &json!({
                "name": "Delete Team",
                "tasks": [
                    {
                        "prompt": "prepare deletion test task"
                    }
                ]
            }),
        )
        ...
        let team_output: serde_json::Value = serde_json::from_str(&created_team).expect("json");
        let team_id = team_output["team_id"].as_str().expect("team id").to_string();
        let task_id = team_output["task_ids"][0]
            .as_str()
            .expect("task id")
            .to_string();

        let deleted = execute_tool("TeamDelete", &json!({ "team_id": team_id }))
            .expect("TeamDelete should succeed");
        let deleted_output: serde_json::Value = serde_json::from_str(&deleted).expect("json");
        assert_eq!(deleted_output["status"], "deleted");
        assert_eq!(deleted_output["stopped_task_count"], 1);

        let task = execute_tool("TaskGet", &json!({ "task_id": task_id }))
            .expect("TaskGet should succeed after TeamDelete");
        let task_output: serde_json::Value = serde_json::from_str(&task).expect("json");
        assert_eq!(task_output["status"], "stopped");
    }

    #[test]
    fn team_create_auto_start_spawns_agents_and_marks_tasks_running() {
        let manifest_dir = temp_path("team-autostart-manifests");
        fs::create_dir_all(&manifest_dir).expect("manifest dir");
        let manifest_dir_for_launch = manifest_dir.clone();
        let mut launched_prompts = Vec::new();
        let mut launch_index = 0_u32;
        let response = run_team_create_with_agent_launcher(
            TeamCreateInput {
                name: "AutoStart Team".to_string(),
                tasks: vec![
                    TeamTaskInput {
                        task_id: None,
                        prompt: Some("lane one prompt".to_string()),
                        description: Some("lane one".to_string()),
                    },
                    TeamTaskInput {
                        task_id: None,
                        prompt: Some("lane two prompt".to_string()),
                        description: None,
                    },
                ],
                auto_start: true,
                auto_start_subagent_type: Some("general-purpose".to_string()),
                auto_start_model: Some("sonnet".to_string()),
            },
            |agent_input| {
                launch_index += 1;
                launched_prompts.push(agent_input.prompt.clone());
                let manifest_file = manifest_dir_for_launch.join(format!("agent_test_{launch_index}.json"));
                let output_file = manifest_dir_for_launch.join(format!("agent_test_{launch_index}.md"));
                fs::write(
                    &manifest_file,
                ...
                fs::write(&output_file, "").expect("write output");
                Ok(AgentOutput {
                    agent_id: format!("agent_test_{launch_index}"),
                    task_id: agent_input.task_id,
                    name: agent_input.name.unwrap_or_else(|| "auto".to_string()),
                    description: agent_input.description,
                    subagent_type: agent_input.subagent_type,
                    model: agent_input.model,
                    status: "running".to_string(),
                    output_file: output_file.display().to_string(),
                    manifest_file: manifest_file.display().to_string(),
                    created_at: "2026-04-08T00:00:00Z".to_string(),
                    started_at: Some("2026-04-08T00:00:00Z".to_string()),
                    completed_at: None,
                    lane_events: Vec::new(),
                    current_blocker: None,
        ...
        .expect("TeamCreate auto_start should succeed");

        let output: serde_json::Value = serde_json::from_str(&response).expect("json");
        assert_eq!(output["execution_mode"], "auto_start");
        assert_eq!(output["auto_start_requested"], true);
        assert_eq!(output["auto_start_started_count"], 2);
        assert_eq!(output["auto_start_failed_count"], 0);
        assert_eq!(output["model_execution_policy"], "single_lane_per_model");
        assert_eq!(output["model_max_parallel_per_model"], 1);
        assert_eq!(output["auto_started_agents"].as_array().unwrap().len(), 2);
        assert_eq!(launched_prompts, vec!["lane one prompt", "lane two prompt"]);

        let task_ids = output["task_ids"]
            .as_array()
            .expect("task ids")
            .iter()
            .map(|value| value.as_str().expect("task id").to_string())
        ...
        for task_id in task_ids {
            let task = execute_tool("TaskGet", &json!({ "task_id": task_id }))
                .expect("TaskGet should succeed after auto-start");
            let task_output: serde_json::Value = serde_json::from_str(&task).expect("json");
            assert_eq!(task_output["status"], "running");
        }
    ...
    #[test]
    fn team_create_auto_start_marks_task_failed_on_manifest_failure() {
        let manifest_dir = temp_path("team-autostart-failure");
        fs::create_dir_all(&manifest_dir).expect("manifest dir");
        let manifest_dir_for_launch = manifest_dir.clone();

        let response = run_team_create_with_agent_launcher(
            TeamCreateInput {
                name: "AutoStart Failure Team".to_string(),
                tasks: vec![TeamTaskInput {
                    task_id: None,
                    prompt: Some("lane failure prompt".to_string()),
                    description: None,
                }],
                auto_start: true,
                auto_start_subagent_type: Some("general-purpose".to_string()),
            ...
            move |agent_input| {
                let manifest_file = manifest_dir_for_launch.join("agent_failed.json");
                let output_file = manifest_dir_for_launch.join("agent_failed.md");
                fs::write(
                    &manifest_file,
                    serde_json::to_string_pretty(&json!({
                        "status": "failed",
                        "derivedState": "truly_idle",
                        "error": "simulated auth failure"
                    }))
                    .expect("manifest json"),
                )
                .expect("write failed manifest");
                fs::write(&output_file, "startup failed").expect("write failed output");
        ...
        .expect("TeamCreate auto_start should succeed with failure details");

        let output: serde_json::Value = serde_json::from_str(&response).expect("json");
        assert_eq!(output["auto_start_started_count"], 0);
        assert_eq!(output["auto_start_failed_count"], 1);
        assert!(output["auto_start_failures"][0]["reason"]
            .as_str()
            .expect("failure reason")
            .contains("simulated auth failure"));

        let task_id = output["task_ids"][0]
            .as_str()
            .expect("task id")
            .to_string();
        let task = execute_tool("TaskGet", &json!({ "task_id": task_id }))
            .expect("TaskGet should succeed after auto-start failure");
        let task_output: serde_json::Value = serde_json::from_str(&task).expect("json");
        assert_eq!(task_output["status"], "failed");
    ...
    #[test]
    fn agent_store_dir_defaults_to_current_directory() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let previous_store = std::env::var("CLAWD_AGENT_STORE").ok();
        std::env::remove_var("CLAWD_AGENT_STORE");

        let cwd = std::env::current_dir().expect("current dir");
        let dir = agent_store_dir().expect("agent store dir should resolve");
        ...
        assert_eq!(dir, cwd.join(".clawd-agents"));
    }
    ...
    #[test]
    fn nested_agent_context_propagates_task_id_and_upstream_targets() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let dir = temp_path("nested-agent-context");
        fs::create_dir_all(&dir).expect("agent store should exist");
        std::env::set_var("CLAWD_AGENT_STORE", &dir);

        let upstream_output = dir.join("parent-output.md");
        fs::write(&upstream_output, "# parent\n").expect("upstream output file should exist");
        let upstream_output_path = upstream_output.display().to_string();

        let captured = Arc::new(Mutex::new(None::<AgentJob>));
        let captured_for_spawn = Arc::clone(&captured);

        let manifest = {
            let _context_guard = super::AgentSpawnContextGuard::enter(super::AgentSpawnContext {
                task_id: Some("task_parent_context".to_string()),
                upstream_output_targets: vec![upstream_output_path.clone()],
            });
            execute_agent_with_spawn(
                AgentInput {
                    description: "Nested child".to_string(),
                    prompt: "Run nested work".to_string(),
                    subagent_type: Some("Explore".to_string()),
                    name: Some("nested-child".to_string()),
                    model: None,
                    task_id: None,
                },
                move |job| {
            ...
            .expect("nested agent should spawn")
        };
        ...
        assert_eq!(manifest.task_id.as_deref(), Some("task_parent_context"));
        assert_eq!(captured_job.manifest.task_id.as_deref(), Some("task_parent_context"));
        assert!(
            captured_job
                .upstream_output_targets
                .iter()
                .any(|target| target == &upstream_output_path),
            "nested job should inherit upstream output targets"
        );
    ...
    #[test]
    fn terminal_state_propagates_to_upstream_output_targets() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let dir = temp_path("nested-output-propagation");
        fs::create_dir_all(&dir).expect("agent store should exist");
        std::env::set_var("CLAWD_AGENT_STORE", &dir);
        ...
        let manifest = {
            let _context_guard = super::AgentSpawnContextGuard::enter(super::AgentSpawnContext {
                task_id: Some("task_parent_context".to_string()),
                upstream_output_targets: vec![upstream_output_path],
            });
            execute_agent_with_spawn(
                AgentInput {
                    description: "Nested child propagation".to_string(),
                    prompt: "Propagate output".to_string(),
                    subagent_type: Some("Explore".to_string()),
                    name: Some("nested-child-propagation".to_string()),
                    model: None,
                    task_id: None,
                },
                move |job| {
        ...
        super::persist_agent_terminal_state_with_targets(
            &manifest,
            "completed",
            Some("nested child done"),
            None,
            &captured_job.upstream_output_targets,
        )
        .expect("persisting child output should succeed");

        let child_contents =
            fs::read_to_string(&manifest.output_file).expect("child output should exist");
        let parent_contents = fs::read_to_string(&upstream_output).expect("parent output should exist");

        assert!(child_contents.contains("nested child done"));
        assert!(parent_contents.contains("nested child done"));
    ...
    #[test]
    fn agent_tool_mapping_inherits_main_toolset() {
        let general = allowed_tools_for_subagent("general-purpose");
        assert!(general.contains("bash"));
        assert!(general.contains("write_file"));
        assert!(general.contains("Agent"));
        assert!(general.contains("TaskCreate"));
        assert!(general.contains("WorkerSendPrompt"));
...
"#,
    ...
    #[test]
    fn agent_runtime_defaults_follow_project_settings_model_and_env() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let root = temp_path("agent-runtime-settings");
        let home = root.join("home");
        let project = root.join("project");
        let nested = project.join("nested").join("workspace");
        fs::create_dir_all(home.join(".claw")).expect("home dir");
        fs::create_dir_all(project.join(".claw")).expect("project settings dir");
  ...
  "env": {
    ...
    "OPENAI_API_KEY": "project-local-openai-key"
  }
}"#,
        )
        .expect("write local settings");
        ...
        let original_home = std::env::var("HOME").ok();
        let original_config_home = std::env::var("CLAW_CONFIG_HOME").ok();
        let original_agent_default_model = std::env::var("CLAWD_AGENT_DEFAULT_MODEL").ok();
        ...
        std::env::set_var("HOME", &home);
        std::env::remove_var("CLAW_CONFIG_HOME");
        std::env::remove_var("CLAWD_AGENT_DEFAULT_MODEL");
        std::env::remove_var("OPENAI_BASE_URL");
        std::env::remove_var("OPENAI_API_KEY");
        std::env::set_current_dir(&nested).expect("set cwd");

        let model = resolve_agent_model(None);
        assert_eq!(model, "google/gemma-4-26b-a4b");

        apply_runtime_env_overrides_for_agent_runtime().expect("runtime env should load");
        assert_eq!(
            std::env::var("OPENAI_BASE_URL").expect("OPENAI_BASE_URL"),
            "http://127.0.0.1:1234/v1"
        );
        assert_eq!(
            std::env::var("OPENAI_API_KEY").expect("OPENAI_API_KEY"),
            "project-local-openai-key"
        );
        ...
        match original_agent_default_model {
            Some(value) => std::env::set_var("CLAWD_AGENT_DEFAULT_MODEL", value),
            None => std::env::remove_var("CLAWD_AGENT_DEFAULT_MODEL"),
        }
    ...
    #[test]
    fn runtime_config_base_discovery_uses_agent_store_ancestors() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let root = temp_path("agent-runtime-config-base");
        let home = root.join("home");
        let workspace = root.join("workspace");
        let config_root = root.join("runtime-project");
        let agent_store = config_root.join(".clawd-agents");
        fs::create_dir_all(home.join(".claw")).expect("home dir");
        fs::create_dir_all(workspace.join("nested")).expect("workspace dir");
        fs::create_dir_all(config_root.join(".claw")).expect("config root");
        fs::create_dir_all(&agent_store).expect("agent store");
        fs::write(
            config_root.join(".claw").join("settings.local.json"),
            r#"{"model":"google/gemma-4-26b-a4b"}"#,
        )
        .expect("write local settings");
        ...
        let original_agent_config_base = std::env::var("CLAWD_AGENT_CONFIG_BASE_DIR").ok();
        let original_dir = std::env::current_dir().expect("cwd");

        std::env::set_var("HOME", &home);
        std::env::remove_var("CLAW_CONFIG_HOME");
        std::env::set_var("CLAWD_AGENT_STORE", &agent_store);
        std::env::remove_var("CLAWD_AGENT_CONFIG_BASE_DIR");
        std::env::set_current_dir(workspace.join("nested")).expect("set cwd");

        let discovered =
            resolve_runtime_config_base_dir_for_agent().expect("runtime base dir should resolve");
        assert_eq!(discovered, config_root);
        ...
        match original_agent_config_base {
            Some(value) => std::env::set_var("CLAWD_AGENT_CONFIG_BASE_DIR", value),
            None => std::env::remove_var("CLAWD_AGENT_CONFIG_BASE_DIR"),
        }
        let _ = fs::remove_dir_all(root);
    }
    ...
    #[test]
    fn task_create_input_defaults_to_auto_start_true() {
        let input: TaskCreateInput = serde_json::from_value(json!({
            "prompt": "default auto start"
        }))
        .expect("TaskCreateInput should deserialize");

        assert!(input.auto_start);
    }

    #[test]
    fn task_create_execute_tool_defaults_to_auto_start_mode() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let dir = temp_path("task-create-execute-tool");
        std::env::set_var("CLAWD_AGENT_STORE", &dir);

        let result = execute_tool(
            "TaskCreate",
            &json!({
                "prompt": "smoke-test default auto start"
            }),
        )
        .expect("TaskCreate should succeed");
        ...
        assert_eq!(output["execution_mode"], "auto_start");
        assert_eq!(output["auto_start_requested"], true);
        assert_ne!(output["status"], "created");
    }

    #[test]
    fn task_create_auto_start_spawns_agent_and_sets_running() {
        let manifest_dir = temp_path("task-create-autostart");
        fs::create_dir_all(&manifest_dir).expect("manifest dir");
        let manifest_dir_for_launch = manifest_dir.clone();

        let response = run_task_create_with_agent_launcher(
            TaskCreateInput {
                prompt: "draft architecture document".to_string(),
                description: Some("Architect Agent".to_string()),
                auto_start: true,
                auto_start_subagent_type: Some("general-purpose".to_string()),
                auto_start_model: Some("sonnet".to_string()),
            },
            move |agent_input| {
                let manifest_file = manifest_dir_for_launch.join("task_autostart_agent.json");
                let output_file = manifest_dir_for_launch.join("task_autostart_agent.md");
                fs::write(
                    &manifest_file,
                    serde_json::to_string_pretty(&json!({
                        "status": "running",
                ...
                .expect("write manifest");
                fs::write(&output_file, "running").expect("write output");

                Ok(AgentOutput {
                    agent_id: "agent_task_autostart".to_string(),
                    task_id: agent_input.task_id,
                    name: agent_input.name.unwrap_or_else(|| "task-auto".to_string()),
                    description: agent_input.description,
                    subagent_type: agent_input.subagent_type,
                    model: agent_input.model,
                    status: "running".to_string(),
        ...
        .expect("TaskCreate auto_start should succeed");

        let output: serde_json::Value = serde_json::from_str(&response).expect("json");
        assert_eq!(output["status"], "running");
        assert_eq!(output["execution_mode"], "auto_start");
        assert_eq!(output["auto_start_requested"], true);
        assert_eq!(output["auto_start_started"], true);
        assert_eq!(
            output["auto_started_agent"]["observed_status"],
            "running"
        );
        assert!(output["auto_start_failure"].is_null());

        let _ = fs::remove_dir_all(manifest_dir);
    }

    #[test]
    fn task_create_auto_start_transitions_to_completed_after_agent_finishes() {
        let manifest_dir = temp_path("task-create-autostart-complete");
        fs::create_dir_all(&manifest_dir).expect("manifest dir");
        let manifest_dir_for_launch = manifest_dir.clone();

        let response = run_task_create_with_agent_launcher(
            TaskCreateInput {
                prompt: "complete after launch".to_string(),
                description: Some("Completing Agent".to_string()),
                auto_start: true,
                auto_start_subagent_type: Some("general-purpose".to_string()),
                auto_start_model: Some("sonnet".to_string()),
            },
            move |agent_input| {
                let manifest_file = manifest_dir_for_launch.join("task_complete_agent.json");
                let output_file = manifest_dir_for_launch.join("task_complete_agent.md");
                fs::write(
                    &manifest_file,
                    serde_json::to_string_pretty(&json!({
                        "status": "running",
                ...
                let manifest = AgentOutput {
                    agent_id: "agent_task_complete".to_string(),
                    task_id: agent_input.task_id,
                    name: agent_input.name.unwrap_or_else(|| "task-complete".to_string()),
                    description: agent_input.description,
                    subagent_type: agent_input.subagent_type,
                    model: agent_input.model,
                    status: "running".to_string(),
                ...
                let completion_manifest = manifest.clone();
                thread::spawn(move || {
        ...
        let output: serde_json::Value = serde_json::from_str(&response).expect("json");
        assert_eq!(output["status"], "running");
        let task_id = output["task_id"].as_str().expect("task id").to_string();
        ...
        loop {
            ...
            assert!(
                ...
                "task should transition to completed"
            );
    ...
    #[test]
    fn task_create_auto_start_can_be_disabled_for_queue_only() {
        let response = run_task_create_with_agent_launcher(
            TaskCreateInput {
                prompt: "queued only".to_string(),
                description: None,
                auto_start: false,
                auto_start_subagent_type: None,
                auto_start_model: None,
            },
            |_agent_input| {
                panic!("launcher should not be called when auto_start is false");
            },
        )
        .expect("TaskCreate queue-only should succeed");

        let output: serde_json::Value = serde_json::from_str(&response).expect("json");
        assert_eq!(output["status"], "created");
        assert_eq!(output["execution_mode"], "manual");
        assert_eq!(output["auto_start_requested"], false);
        assert_eq!(output["auto_start_started"], false);
    }

    #[test]
    fn task_create_auto_start_marks_failed_when_manifest_reports_failure() {
        let manifest_dir = temp_path("task-create-failed");
        fs::create_dir_all(&manifest_dir).expect("manifest dir");
        let manifest_dir_for_launch = manifest_dir.clone();
        ...
        let response = run_task_create_with_agent_launcher(
            TaskCreateInput {
                ...
                description: Some("Failing Agent".to_string()),
                auto_start: true,
                auto_start_subagent_type: Some("general-purpose".to_string()),
                auto_start_model: Some("sonnet".to_string()),
            },
            move |agent_input| {
                let manifest_file = manifest_dir_for_launch.join("task_failed_agent.json");
                let output_file = manifest_dir_for_launch.join("task_failed_agent.md");
                fs::write(
                    &manifest_file,
                    serde_json::to_string_pretty(&json!({
                        "status": "failed",
                ...
                .expect("write failed manifest");
                fs::write(&output_file, "failed").expect("write failed output");

                Ok(AgentOutput {
                    agent_id: "agent_task_failed".to_string(),
                    task_id: agent_input.task_id,
                    name: agent_input.name.unwrap_or_else(|| "task-failed".to_string()),
                    description: agent_input.description,
                    subagent_type: agent_input.subagent_type,
                    model: agent_input.model,
                    status: "running".to_string(),
        ...
        .expect("TaskCreate auto_start should return failure details");

        let output: serde_json::Value = serde_json::from_str(&response).expect("json");
        assert_eq!(output["status"], "failed");
        assert_eq!(output["auto_start_started"], false);
        assert!(output["auto_start_failure"]["reason"]
            .as_str()
            .expect("failure reason")
            .contains("simulated startup failure"));

        let _ = fs::remove_dir_all(manifest_dir);
    }

    #[test]
    fn task_update_auto_activates_created_task_by_default() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let dir = temp_path("task-update-activate-default");
        std::env::set_var("CLAWD_AGENT_STORE", &dir);

        let created = execute_tool(
            "TaskCreate",
            &json!({
                "prompt": "queued then activate",
                "auto_start": false
            }),
        )
        .expect("TaskCreate should succeed");
        let created_output: serde_json::Value = serde_json::from_str(&created).expect("json");
        let task_id = created_output["task_id"]
            .as_str()
            .expect("task id")
            .to_string();
        ...
        let updated_output: serde_json::Value = serde_json::from_str(&updated).expect("json");
        assert_eq!(updated_output["activation_attempted"], true);
        assert_ne!(updated_output["status"], "created");
        assert!(updated_output["activation_started"] == true || !updated_output["activation_failure"].is_null());
    }

    #[test]
    fn task_update_can_skip_auto_activation() {
        let created = execute_tool(
            "TaskCreate",
            &json!({
                "prompt": "queued keep idle",
                "auto_start": false
            }),
        )
        .expect("TaskCreate should succeed");
        ...
        let updated = execute_tool(
            ...
            &json!({
                "task_id": task_id,
                "message": "note only",
                "auto_activate_created": false
            }),
        )
        .expect("TaskUpdate should succeed");

        let updated_output: serde_json::Value = serde_json::from_str(&updated).expect("json");
        assert_eq!(updated_output["activation_attempted"], false);
        assert_eq!(updated_output["activation_started"], false);
        assert_eq!(updated_output["status"], "created");
    }

    #[test]
    fn task_update_auto_activates_failed_task_by_default() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let dir = temp_path("task-update-activate-failed");
        std::env::set_var("CLAWD_AGENT_STORE", &dir);

        let created = execute_tool(
            "TaskCreate",
            &json!({
                "prompt": "queued then mark failed",
                "auto_start": false
            }),
        )
        .expect("TaskCreate should succeed");
        ...
        assert_eq!(updated_output["activation_attempted"], true);
        assert!(updated_output["activation_started"] == true || !updated_output["activation_failure"].is_null());
        assert!(updated_output["activation_skipped_reason"].is_null());
    }
