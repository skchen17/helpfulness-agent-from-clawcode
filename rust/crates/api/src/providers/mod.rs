...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    if has_non_empty_env("OPENAI_BASE_URL") && openai_compat::has_api_key("OPENAI_API_KEY") {
...
mod tests {
    ...
    #[test]
    fn explicit_openai_base_url_overrides_generic_anthropic_fallback_for_unknown_models() {
        let _guard = env_lock();
        ...
        std::env::set_var("OPENAI_BASE_URL", "http://127.0.0.1:1234/v1");
        std::env::set_var("OPENAI_API_KEY", "lm-studio");
        std::env::set_var("ANTHROPIC_API_KEY", "sk-ant-priority-test");

        let provider = detect_provider_kind("qwen2.5-coder-7b-instruct");
