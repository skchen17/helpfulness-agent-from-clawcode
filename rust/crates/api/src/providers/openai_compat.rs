...
fn translate_message(message: &InputMessage) -> Vec<Value> {
    match message.role.as_str() {
        ...
        _ => {
            ...
            let mut multimodal_blocks = Vec::new();
            let mut has_image = false;

            let flush_user_message = |translated: &mut Vec<Value>,
                                      user_text: &mut String,
                                      multimodal_blocks: &mut Vec<Value>,
                                      has_image: &mut bool| {
                if multimodal_blocks.is_empty() {
                    return;
                }
                if *has_image {
                    translated.push(json!({
                        "role": "user",
                        "content": Value::Array(std::mem::take(multimodal_blocks)),
                    }));
                    user_text.clear();
                } else {
                    translated.push(json!({
                        "role": "user",
                        "content": std::mem::take(user_text),
                    }));
                    multimodal_blocks.clear();
                }
                *has_image = false;
            };
...
fn image_source_to_data_url(source: &ImageSource) -> String {
    match source {
...
fn openai_tool_definition(tool: &ToolDefinition) -> Value {
    let parameters = normalize_openai_tool_parameters(tool.input_schema.clone());
    json!({
        "type": "function",
        "function": {
            "name": tool.name,
...
mod tests {
    ...
    fn request_translation_uses_openai_compatible_shape() {
        ...
        assert_eq!(payload["messages"][1]["content"][0]["type"], json!("text"));
        assert_eq!(
            payload["messages"][1]["content"][1]["type"],
            json!("image_url")
        );
