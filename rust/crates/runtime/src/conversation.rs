    ...
    T: ToolExecutor,
{
    ...
    pub fn run_turn_with_message(
        ...
        user_input_summary: impl Into<String>,
        mut prompter: Option<&mut dyn PermissionPrompter>,
    ) -> Result<TurnSummary, RuntimeError> {
        if user_message.role != MessageRole::User {
            return Err(RuntimeError::new(
                "run_turn_with_message expects a user-role message",
            ));
        }
        if user_message.blocks.is_empty() {
            return Err(RuntimeError::new(
                "run_turn_with_message requires at least one content block",
            ));
        }

        let user_input = user_input_summary.into();
