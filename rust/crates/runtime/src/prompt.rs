...
const MAX_MEMORY_FIELD_CHARS: usize = 320;
const MAX_MEMORY_MILESTONES: usize = 5;
const MAX_MEMORY_NODE_SNIPPETS: usize = 3;
const MAX_MEMORY_NODE_SNIPPET_CHARS: usize = 220;
...
impl SystemPromptBuilder {
    ...
    pub fn build(&self) -> Vec<String> {
        ...
        if let Some(project_context) = &self.project_context {
            ...
            if let Some(memory_section) = render_memory_context_section(&project_context.cwd) {
...
fn render_memory_context_section(cwd: &Path) -> Option<String> {
    let memory_root = discover_memory_root(cwd)?;
    let state_path = memory_root.join("state.json");
    let state_raw = fs::read_to_string(&state_path).ok()?;
    let state: Value = serde_json::from_str(&state_raw).ok()?;

    let global_context = state.get("global_context").and_then(Value::as_object);
    let active_focus = global_context
        .and_then(|context| context.get("active_focus"))
        .and_then(Value::as_str)
        .and_then(|value| normalize_memory_text(value, MAX_MEMORY_FIELD_CHARS));
    let active_tasks = global_context
        .and_then(|context| context.get("active_tasks"))
        .and_then(Value::as_str)
        .and_then(|value| normalize_memory_text(value, MAX_MEMORY_FIELD_CHARS));
    let next_instruction = global_context
        .and_then(|context| context.get("next_instruction"))
        .and_then(Value::as_str)
        .and_then(|value| normalize_memory_text(value, MAX_MEMORY_FIELD_CHARS));
    let active_skills = global_context
        .and_then(|context| context.get("active_skills"))
        .map(extract_memory_string_array)
        .unwrap_or_default();
    let key_nodes = global_context
        .and_then(|context| context.get("key_knowledge_nodes"))
        .map(extract_memory_string_array)
        .unwrap_or_default();

    let recent_milestones = state
        .get("recent_milestones")
        .map(extract_memory_string_array)
        .unwrap_or_default()
        .into_iter()
        .rev()
        .take(MAX_MEMORY_MILESTONES)
        .collect::<Vec<_>>()
        .into_iter()
        .rev()
        .collect::<Vec<_>>();

    let node_snippets = key_nodes
        .iter()
        .take(MAX_MEMORY_NODE_SNIPPETS)
        .filter_map(|node| {
            read_memory_node_snippet(&memory_root, node)
                .map(|snippet| format!("- {node}: {snippet}"))
        })
        .collect::<Vec<_>>();

    if active_focus.is_none()
        && active_tasks.is_none()
        && next_instruction.is_none()
        && active_skills.is_empty()
        && recent_milestones.is_empty()
        && node_snippets.is_empty()
    {
        return None;
    }

    let mut lines = vec!["# Long-term memory context".to_string()];
    lines.extend(prepend_bullets(vec![
        "Loaded from .memory/state.json when available.".to_string(),
        "Treat this as historical context; explicit user instructions take priority.".to_string(),
    ]));
    ...
    if !recent_milestones.is_empty() {
        ...
        lines.extend(
            recent_milestones
                .into_iter()
                .filter_map(|value| normalize_memory_text(&value, MAX_MEMORY_FIELD_CHARS))
                .map(|value| format!("- {value}")),
        );
    }

    if !node_snippets.is_empty() {
        lines.push(String::new());
        lines.push("Knowledge snippets:".to_string());
        lines.extend(node_snippets);
    }

    Some(lines.join("\n"))
}

fn discover_memory_root(cwd: &Path) -> Option<PathBuf> {
    for directory in cwd.ancestors() {
        let candidate = directory.join(".memory");
        if candidate.is_dir() {
...
fn extract_memory_string_array(value: &Value) -> Vec<String> {
    value
        .as_array()
        .map(|entries| {
            entries
                ...
                .filter_map(|entry| normalize_memory_text(entry, MAX_MEMORY_FIELD_CHARS))
                .collect::<Vec<_>>()
        })
        .unwrap_or_default()
}

fn normalize_memory_text(value: &str, max_chars: usize) -> Option<String> {
    let collapsed = value
        .split_whitespace()
...
fn read_memory_node_snippet(memory_root: &Path, node: &str) -> Option<String> {
    let sanitized = sanitize_memory_node_name(node)?;
    let node_path = memory_root
        .join("knowledge_base")
        .join(format!("{sanitized}.md"));
    let content = fs::read_to_string(node_path).ok()?;
    normalize_memory_text(&content, MAX_MEMORY_NODE_SNIPPET_CHARS)
}

fn sanitize_memory_node_name(node: &str) -> Option<String> {
    let sanitized = node
        .chars()
...
",
    ...
    #[test]
    fn renders_memory_context_when_state_exists() {
        let root = temp_dir();
        let memory_root = root.join(".memory");
        fs::create_dir_all(memory_root.join("knowledge_base")).expect("memory kb dir");
        fs::write(
            memory_root.join("state.json"),
            serde_json::to_string_pretty(&json!({
                "global_context": {
                    "active_focus": "Integrate Rust runtime with long-term memory",
                    "active_tasks": "1. Wire startup memory loading\n2. Persist milestones",
                    "next_instruction": "Spawn sub-agents for verification",
                    "active_skills": ["orchestration"],
                    "key_knowledge_nodes": ["rust-integration"]
                },
                "recent_milestones": [
                    "Initialized memory pipeline",
        ...
        fs::write(
            memory_root
                .join("knowledge_base")
                .join("rust-integration.md"),
            "Use runtime prompt context plus turn-level state updates.",
        )
        .expect("write knowledge node");
        ...
        assert!(prompt.contains("Integrate Rust runtime with long-term memory"));
        assert!(prompt.contains("Initialized memory pipeline"));
        assert!(prompt.contains("Use runtime prompt context plus turn-level state updates."));
