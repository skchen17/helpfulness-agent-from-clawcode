...
fn apply_runtime_env_overrides_for_cwd(cwd: &Path) {
...
fn run_mcp_serve() -> Result<(), Box<dyn std::error::Error>> {
    Err("`claw mcp-serve` is not available in this build.".into())
}
...
fn run_stale_base_preflight(flag_value: Option<&str>) {
    if flag_value.is_some() {
        eprintln!("warning: stale-base preflight is not available in this build; continuing.");
    }
}
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let dir = cwd.join(".claw").join("sessions");
        ...
        for block in &message.blocks {
            match block {
                ...
                ContentBlock::Image { media_type, data } => {
                    lines.push(format!(
                        "[image media_type={media_type} base64_bytes={}]",
                        data.len()
                    ));
                }
                ContentBlock::ToolResult {
    ...
    for (index, message) in session.messages.iter().enumerate() {
        ...
        for block in &message.blocks {
            match block {
                ...
                ContentBlock::Image { media_type, data } => {
                    lines.push(format!("![image](data:{media_type};base64,{data})"));
                    lines.push(String::new());
                }
                ContentBlock::ToolUse { id, name, input } => {
                    lines.push(format!(
...
mod tests {
    ...
    #[test]
    fn applies_runtime_config_env_when_process_env_is_missing() {
        let _guard = env_lock();
        let root = temp_dir();
        let cwd = root.join("project");
        let config_home = root.join("config-home");
        ...
        std::fs::write(
            ...
            r#"{"env":{"OPENAI_API_KEY":"config-openai-key","OPENAI_BASE_URL":"https://example.test/v1"}}"#,
        )
        .expect("project local config should write");
        ...
        super::apply_runtime_env_overrides_for_cwd(&cwd);

        let resolved_api_key = std::env::var("OPENAI_API_KEY").ok();
        let resolved_base_url = std::env::var("OPENAI_BASE_URL").ok();
    ...
    #[test]
    fn runtime_config_env_does_not_override_non_empty_process_env() {
        let _guard = env_lock();
        let root = temp_dir();
        let cwd = root.join("project");
        let config_home = root.join("config-home");
        ...
        std::fs::write(
            cwd.join(".claw").join("settings.local.json"),
            r#"{"env":{"OPENAI_API_KEY":"config-openai-key"}}"#,
        )
        .expect("project local config should write");
        ...
        let original_openai_api_key = std::env::var("OPENAI_API_KEY").ok();
        std::env::set_var("CLAW_CONFIG_HOME", &config_home);
        std::env::set_var("OPENAI_API_KEY", "shell-openai-key");
        ...
        assert_eq!(resolved_api_key.as_deref(), Some("shell-openai-key"));
    }
