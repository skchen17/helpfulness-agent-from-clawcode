...
Set credentials for your target provider:
...
# OpenAI-compatible providers (OpenAI, LM Studio, local gateways)
export OPENAI_API_KEY="lm-studio"
export OPENAI_BASE_URL="http://127.0.0.1:1234/v1"
```

You can also place provider env values in runtime config:
...
- media input: `/image [path ...]` (omit `path` to open a GUI picker for image+text input; supports batch paths)
...
- `/doctor`
- `/image [path ...]` (OpenAI-compatible providers)
...
## Team Auto-Start Runtime Notes

- `TaskCreate` defaults to `auto_start: true`, launches a worker automatically, and probes the manifest to set observed task state.
- `TaskUpdate` now supports `auto_activate_created` (default `true`): updating a `created` task will attempt activation and return activation diagnostics.
- `TeamCreate` with `auto_start: true` now validates launch state from each agent manifest instead of trusting spawn return alone.
- Team task status is updated from observed manifest status:
  - `running` => task `running`
  - `completed` => task `completed`
  - `failed` => task `failed` + failure reason in `auto_start_failures`
- Agent manifests are written under `.clawd-agents` by default (or `CLAWD_AGENT_STORE` when set).
- Because most deployments use one underlying model for all lanes, agent runtime enforces per-model concurrency by default:
  - env: `CLAWD_AGENT_MAX_PARALLEL_PER_MODEL`
  - default: `1` (serialized lanes per model)
  - TeamCreate output includes `model_execution_policy` and `model_max_parallel_per_model` for observability.
